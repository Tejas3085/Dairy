...
export class OrganizationComponent implements OnInit {
  ...
  OrgTypeChange(event) {
    console.log('otc:', event.target.value);
  ...
  getAllOrgTypes() {
    this.service.GetAllOrgTypes().subscribe((result) => {
      ...
      console.log("orgtype" + this.OrgTypes);
    });
  }
  ...
  CityChange(event) {
    console.log('CityId:', event.target.value);
  ...
  OnSubmit() {
    ...
    this.service.AddOrganization(this.organizationMaster).subscribe((result) => {
      if (result > 0) {
        ...
        this.service.SaveOrganizationImage(formData,result).subscribe(data => {
